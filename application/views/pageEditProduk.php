	...
	<body>
							...
							<div class="col-xl-7 ml-2">
								...
								<div class="form-group row">
									...
									<div class="col-xl-4">
										...
										$data_sub_kategori = $this->db->get_where('t_sub_kategori', ['id_kategori' => $id_kategori])->result();
										?>
										<select id="select-sub-kategori" class="form-control" name="idSubKategoriBarang">
											<?php foreach($data_sub_kategori as $data): ?>
												<?php
												if($data->id == $produk->id_full_kategori){
													$selected = 'selected';
												} else {
													$selected = '';
												}
												?>
												<option value="<?= $data->id ?>"<?= $selected ?>><?= $data->sub_kategori ?></option>
											<?php endforeach; ?>									
										</select>
									</div>
								</div>
									...
									<div class="col-xl-3">
										...
										<select id="select-lokasi" class="form-control" name="idLokasiBarang">
											...
											<?php foreach($data_lokasi as $data): ?>
												<?php
												if($data->id == $produk->id_lokasi){
													$selected = 'selected';
												} else {
													$selected = '';
												}
												?>
												<option value="<?= $data->id ?>" <?= $selected ?>><?= $data->lokasi ?></option>
											<?php endforeach; ?>
										</select>
									</div>
									<div class="col-xl-3">
										...
										$data_sub_lokasi = $this->db->get_where('t_sub_lokasi', ['id_lokasi' => $id_lokasi])->result();
										?>
										<select id="select-sub-lokasi-1" class="form-control" name="subLokasiBarang1">
											<?php foreach($data_sub_lokasi as $data): ?>
												<?php
												if($data->id == $produk->id_full_lokasi){
													$selected = 'selected';
												} else {
													$selected = '';
												}
												?>
												<option value="<?= $data->id ?>"<?= $selected ?>><?= $data->sub_lokasi_1 ?></option>
											<?php endforeach; ?>
										</select>
									</div>
									<div class="col-xl-3">
										<select id="select-sub-lokasi-2" class="form-control" name="subLokasiBarang2">
											<?php foreach($data_sub_lokasi as $data): ?>
												...
												?>
												<option value="<?= $data->sub_lokasi_2 ?>"<?= $selected ?>><?= $data->sub_lokasi_2 ?></option>
											<?php endforeach; ?>
										</select>
									</div>
								</div>
								...
								<div class="form-group row">
									...
									<div class="col-xl-4">
										<select class="form-control" name="infoBarang">
											<option value=""></option>
											<?php $infoBarang = ['Sudah Tes', 'Belum Tes'] ?>
											<?php foreach($infoBarang as $i): ?>
												<?php
												if($i == $produk->info_barang){
													$selected = 'selected';
												} else {
													$selected = '';
												}
		...
		<script>
			...
			function loadSubKategori(id) {
				$.get("<?= base_url('produk/edit_load_sub_kategori/') ?>" + id, function (data) {
					$('#select-sub-kategori').html(data);
				});
			}

			function loadSubLokasi1(id) {
				$.get("<?= base_url('produk/edit_load_sub_lokasi_1/') ?>" + id, function (data) {
					$('#select-sub-lokasi-1').html(data);
				});
			}

			function loadSubLokasi2(id, subLokasi1) {
				$.get(`<?= base_url('produk/edit_load_sub_lokasi_2/') ?>${id}/${subLokasi1}`, function (data) {
